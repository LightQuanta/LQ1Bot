...
namespace LQ1Bot.Plugins {
    class Meme : PluginBase, IGroupMessage, IFriendMessage {
        ...
        private readonly Dictionary<long, string> LastMeme = new Dictionary<long, string>();

        private readonly Dictionary<long, (string, string)> Rep = new Dictionary<long, (string, string)>();
        ...
        public async Task<bool> FriendMessage(MiraiHttpSession session, IFriendMessageEventArgs e) {
            ...
            if (Regex.IsMatch(text, @"^setrep .+#.+$")) {
                ...
                if (AllRep != null) {
                    ...
                    await session.SendFriendMessageAsync(q, new PlainMessage("未发现该回复内容！"));
                } else {
                    await session.SendFriendMessageAsync(q, new PlainMessage("未发现该回复！"));
                }
                return true;
            }
            #endregion
            ...
            if (Regex.IsMatch(text.ToLower(), @"^getmeme .+")) {
                ...
                if (memes != null) {
                    ...
                    await session.SendFriendMessageAsync(e.Sender.Id, new PlainMessage("未发现该回复"));
                }
                return true;
            }
            ...
            if (text == "来点hiiro" || text == "來點hiiro") {
                ...
                await session.SendFriendMessageAsync(q, new VoiceMessage(null, url, null));
                return true;
            }
            if (MemeMgr.HasReply(text)) {
                var rep = MemeMgr.GetReply(text, q);
                if (rep != null) {
                    LastMeme.Remove(q);
                    LastMeme.Add(q, text);
                    if (rep.StartsWith("[picture]") && rep.Length > 9) {
                        await session.SendImageToFriendAsync(q, new string[] { $"http://127.0.0.1:23333/botpicture/{rep[9..]}" });
                    } else {
                        int n = await session.SendFriendMessageAsync(q, new PlainMessage(rep));
                    }
                    return true;
                }
            }
            if (text.ToLower() == "guid") {
                await session.SendFriendMessageAsync(q, new PlainMessage(Guid.NewGuid().ToString()));
                return true;
            }
            if (text.ToLower() == "来点原神kfc") {
                LastMeme.Remove(q);
                LastMeme.Add(q, text);
                await session.SendFriendMessageAsync(q, new VoiceMessage(null, @"https://img.nga.178.com/attachments/mon_202103/08/i2Qj0k-i7puZg.mp3?duration=40%A1%E5%E2%80%8B", null));
                return true;
            }
            #endregion
            return false;
        ...
        public async Task<bool> GroupMessage(MiraiHttpSession session, IGroupMessageEventArgs e) {
            ...
            #region meme添加管理
            if (MemeMgr.IsAdmin(e.Sender.Id) && text == "y") {
            ...
            if (Regex.IsMatch(text.ToLower(), @"^getmemejson .+")) {
                ...
                if (json != null) {
                    ...
                    await session.SendGroupMessageAsync(q, new PlainMessage("未发现该回复"));
                }
                return true;
            }
            if (text == "savememe") {
